...
model_cnn = torch.load('./models/M2_model.pth', map_location=torch.device('cpu'))
...
def enc_test(evaluator, ckks_encoder, galois_key, relin_keys, csps_ctxt, csps_conv_weights, csps_conv_biases, image_size, paddings, strides, data_size, label):
    ...
    print('FLATTEN TIME\t%.3f' %(CHECK_TIME7-CHECK_TIME6))
    ...
    END_TIME = time.time()
    print('FC 1 TIME\t%.3f' %(END_TIME-CHECK_TIME7))
